...
\begin{frame}[plain]
  ...
  \begin{beamercolorbox}[rounded=true,sep=0.7em,wd=0.91\textwidth]{progress remaining}
    \textbf{Current position:} the E0--E10 software, provenance, replay, and analysis path is implemented; live Qwen validation and every Qwen scientific phase remain to be executed.
  \end{beamercolorbox}
  \vspace{0.4cm}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      ...
      \begin{enumerate}\setlength\itemsep{0.06cm}
        ...
        \item The study moved to one native CUDA checkpoint on a single A100 40GB.
      \end{enumerate}
    \end{column}
    \begin{column}{0.47\textwidth}
...
\begin{frame}{Uniform four-bit Qwen is the frozen scientific target}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \texttt{nvidia/Qwen3.6-27B-NVFP4}\par
      exact revision \texttt{c000ac2c...3282}\par
      vLLM 0.24.0 with ModelOpt mixed loading\par
      NVIDIA A100-SXM4 40GB, SM80\par\vspace{0.35cm}
      64 blocks, hidden size 5,120; pinned 21.94 GB snapshot; mixed FP8/NVFP4 representation of \texttt{Qwen/Qwen3.6-27B}.
    \end{column}
    \begin{column}{0.46\textwidth}
      \textbf{Why this model}\par\vspace{0.15cm}
      \begin{itemize}\setlength\itemsep{0.13cm}
        \item It fits the approved A100 40GB with headroom for inference state.
        \item Native vLLM uses the released NVIDIA ModelOpt representation.
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      ...
      \begin{itemize}\setlength\itemsep{0.04cm}
        ...
        \item throughput, latency, and peak GPU memory
...
\begin{frame}{Viability is established; efficacy remains open}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.31\textwidth}
      ...
      Run live A100 preflight, Qwen E0, then a fresh Qwen E1 in the active namespace.
    \end{column}
    \begin{column}{0.31\textwidth}
...
\begin{frame}{Next: complete the evidence chain}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.47\textwidth}
      ...
      Download and verify the exact snapshot, run live A100 hook preflight, then execute and finalize a fresh Qwen E0.\par\vspace{0.25cm}
